...
class MC(Problem):
    def actionHelper(self, action, state):
        newState = self.result(state, action)
        if (newState[0][1] > newState[0][0]) and newState[0][0] > 0:
            return False
        if (newState[1][1] > newState[1][0]) and newState[1][0] > 0:
            return False
        else:
            return True

    def __init__(self, m, c):
        self.initial = ((m,c),(0,0),0)
        self.goal = ((0,0),(m,c),1)
    ...
    def actions(self, state):
        ...
        #Ugly ifs, pretty sure this is the best way to do it though
        if(boat == 0):
            if cannL:
                actionList.append("c")
                if cannL >= 2:
                    actionList.append("cc")
            if missL:
                actionList.append("m")
                if missL >= 2:
                    actionList.append("mm")
            if missL >= 1 and cannL >= 1:
                actionList.append("mc")
        else:
            if cannR:
                actionList.append("c")
                if cannR >= 2:
            ...
            if missR:
                ...
                if missR >= 2:
                    actionList.append("mm")
            if missR >= 1 and cannR >= 1:
                actionList.append("mc")

        #FUNCTIONAL
        actionList = list(filter((lambda x : self.actionHelper(x, state)), actionList))
        return actionList
    ...
    def result(self, state, action):
        ...
        #This is ugly code, I should have used a switch but alas, I didn't
        if boat == 0:
            if action == "c":
                cannL -= 1; cannR += 1
            elif action == "cc":
                ...
                cannL -= 1; missL -= 1; cannR += 1; missR += 1
            boat = 1
        else:
            if action == "c":
                cannL += 1;cannR -= 1
                ...
                cannL += 1;missL += 1;cannR -= 1;missR -= 1
            boat = 0
        newstate = ((missL, cannL),(missR, cannR), boat)
        return newstate
...
def breadth_first_tree_search(problem):
    ...
    return tree_search(problem, FIFOQueue()).path()
